---
name: "25-test-e2e"
description: "扫描项目源码，自动识别可测试的页面和用户流程，编写并运行 Playwright E2E 测试代码，模拟真实用户在浏览器中的完整操作流程，生成测试报告。当用户提到"E2E测试"、"端到端测试"、"用户流程测试"、"UI自动化测试"、"Playwright测试"、"写E2E测试"、"自动化测试"时使用。"
---
...
**推荐**：Playwright（支持多浏览器、自动等待、截图/视频、网络拦截）。其他选项参见 `examples/e2e-test-examples.md`。
...
| 脚本 | 用途 | 用法 |
|------|------|------|
| `scan_pages.py` | 扫描项目中的可测试页面和用户流程 | `python resources/scan_pages.py <项目路径> [输出JSON]` |
| `generate_e2e_tests.py` | 根据扫描结果生成 E2E 测试代码 | `python resources/generate_e2e_tests.py <扫描结果JSON> <输出目录>` |
| `run_e2e_tests.py` | 运行 E2E 测试并收集结果 | `python resources/run_e2e_tests.py <项目路径> [--headed]` |
...
```bash
python resources/scan_pages.py <项目路径> ./e2e_scan_result.json
```
...
```bash
python resources/generate_e2e_tests.py ./e2e_scan_result.json ./generated_e2e_tests/
```
...
```bash
python resources/run_e2e_tests.py <项目路径> [--headed] [--browser chromium]
```
...
```bash
python resources/generate_report.py ./e2e_test_result.json docs/测试报告/E2E测试报告.md ./e2e_scan_result.json
```
...
> 完整示例（登录、注册、搜索、响应式测试）请参阅 [examples/e2e-test-examples.md](examples/e2e-test-examples.md)。

```typescript
// ✅ Playwright E2E 测试简短示例
