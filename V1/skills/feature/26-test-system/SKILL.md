---
name: "26-test-system"
description: "系统测试技能。当用户提到"系统测试"、"system test"、"全链路测试"、"端到端系统测试"、"性能测试"、"负载测试"、"安全审计"、"安全扫描"、"漏洞扫描"、"兼容性测试"、"跨浏览器测试"、"数据备份测试"、"健康检查"、"系统监控"、"CI/CD 测试"时使用此技能。AI 会扫描项目架构，自动生成并运行功能/性能/安全/兼容性/备份恢复测试，生成系统测试报告。"
...
| 脚本 | 用途 | 用法 |
|------|------|------|
| `scan_system.py` | 扫描系统架构（服务/数据库/缓存/消息队列/外部集成/安全） | `python scripts/scan_system.py <项目路径> [输出JSON]` |
| `generate_system_tests.py` | 生成 6 类测试代码骨架 | `python scripts/generate_system_tests.py <扫描结果JSON> <输出目录>` |
| `run_system_tests.py` | 运行系统测试并收集结果 | `python scripts/run_system_tests.py <项目路径> [--type all\|functional\|performance\|security\|health] [--output <JSON>]` |
| `generate_report.py` | 生成 Markdown 格式系统测试报告 | `python scripts/generate_report.py <测试结果JSON> <报告路径> [扫描结果JSON]` |
...
**scan_system.py**
```
python scripts/scan_system.py <项目路径> [输出JSON路径]
```
- `<项目路径>`（必填）：项目根目录路径
- `[输出JSON路径]`（可选）：扫描结果输出文件路径，不指定则仅打印到 stdout

**generate_system_tests.py**
```
python scripts/generate_system_tests.py <扫描结果JSON> <输出目录>
```
- `<扫描结果JSON>`（必填）：scan_system.py 的输出 JSON 文件路径
- `<输出目录>`（必填）：测试文件输出目录

**run_system_tests.py**
```
python scripts/run_system_tests.py <项目路径> [--type TYPE] [--output JSON]
```
- `<项目路径>`（必填）：项目根目录路径
- `--type, -t`（可选）：测试类型，可选值 `all`（默认）、`functional`、`performance`、`security`、`health`
- `--output, -o`（可选）：测试结果输出 JSON 路径

**generate_report.py**
```
python scripts/generate_report.py <测试结果JSON> <报告路径> [扫描结果JSON]
```
- `<测试结果JSON>`（必填）：run_system_tests.py 的输出 JSON 文件路径
- `<报告路径>`（必填）：Markdown 报告输出路径
- `[扫描结果JSON]`（可选）：scan_system.py 的输出 JSON，用于生成架构概览章节

### 输出文件格式

**scan_system.py 输出 JSON 格式：**
```json
{
  "services_count": 3,
  "database_count": 1,
  ...
  "security_findings": [
    {"name": ".gitignore 缺少 .env", "severity": "medium", "detail": "..."}
  ],
  "env_vars": ["DB_HOST", "API_KEY"],
  "components": [
    {"name": "Frontend (React)", "file": "package.json", "type": "frontend", "category": "service"},
    {"name": "Backend (Express)", "file": "package.json", "type": "backend", "category": "service"},
    {"name": "PostgreSQL", "type": "relational", "file": "docker-compose.yml", "category": "database"}
  ]
}
```

**run_system_tests.py 输出 JSON 格式：**
```json
{
  "test_type": "all",
  "passed": 15,
  ...
  "functional": {"framework": "playwright", "passed": 10, "failed": 2, "total": 12},
  "security": {"framework": "npm-audit", "vulnerabilities": {"critical": 0, "high": 1, "moderate": 2, "low": 0, "info": 0}},
  "performance": {"framework": "k6", "metrics": {}},
  "health": {"framework": "playwright", "passed": 3, "failed": 0, "total": 3}
}
```
...
**前端框架：** React, Vue, Next.js, Nuxt.js, Svelte

**后端框架：** Express, NestJS, Fastify, Flask, Django, FastAPI, Java/Spring, Go, Rust, C#/.NET

**数据库：** MySQL, PostgreSQL, MongoDB, SQLite, Elasticsearch, Prisma, Sequelize, Mongoose

**缓存：** Redis, Memcached

**消息队列：** RabbitMQ, Apache Kafka, NATS, Redis Streams, Bull, Celery, Sidekiq, ActiveMQ, AWS SQS, Google Pub/Sub, Azure Event Hub

**外部集成（30+）：** Stripe, PayPal, Razorpay, 支付宝, 微信支付, AWS, Firebase, Supabase, SendGrid, Nodemailer, Twilio, AWS S3, Cloudinary, Google API, Facebook API, GitHub API, OAuth, JWT, Sentry, Datadog, Elasticsearch, OpenAI, Anthropic, LangChain, Redis Cloud, Upstash, Vercel, Netlify, Docker, Kubernetes, Grafana, Prometheus, Loki, MinIO, Cloudflare
...
```bash
python scripts/scan_system.py <项目路径> /data/user/work/system_scan.json
```
...
```bash
python scripts/generate_system_tests.py /data/user/work/system_scan.json /data/user/work/system-tests/
```
...
```bash
# 运行全部系统测试
python scripts/run_system_tests.py <项目路径> --output /data/user/work/system_result.json

# 仅运行特定类型
python scripts/run_system_tests.py <项目路径> --type functional
python scripts/run_system_tests.py <项目路径> --type performance
python scripts/run_system_tests.py <项目路径> --type security
python scripts/run_system_tests.py <项目路径> --type health
```
...
```bash
python scripts/generate_report.py /data/user/work/system_result.json docs/测试报告/系统测试报告.md /data/user/work/system_scan.json
```
...
| 组件 | 数量 | 详情 |
|------|------|------|
| 服务组件 | X | Frontend (React), Backend (Express), ... |
| 数据库 | X | PostgreSQL, Redis |
| 消息队列 | X | RabbitMQ, ... |
| 外部集成 | X | Stripe, SendGrid, ... |
| 配置文件 | X | - |
...
| 指标 | 基线 | 实际 | 状态 |
|------|------|------|------|
| API P95 响应时间 | < 500ms | Xms | ✅/❌ |
| API P99 响应时间 | < 1000ms | Xms | ✅/❌ |
| 并发失败率 | < 1% | X% | ✅/❌ |
...
jobs:
  system-test:
    ...
    services:
      postgres:
        image: postgres:15  # 版本号请根据项目实际使用的 PostgreSQL 版本调整
        env:
          POSTGRES_DB: testdb
          POSTGRES_PASSWORD: testpass
        ports: ["5432:5432"]
      redis:
        image: redis:7  # 版本号请根据项目实际使用的 Redis 版本调整
        ports: ["6379:6379"]
...
system_test:
  stage: test
  image: mcr.microsoft.com/playwright:v1.40.0-jammy  # 版本号请根据项目实际使用的 Playwright 版本调整
  services:
    - postgres:15
    - redis:7
  script:
