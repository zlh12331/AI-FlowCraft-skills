...
description: "项目初始化执行者。读取 specs/ 下的定义文档，自动创建目录结构、配置文件和初始化 Git 仓库。触发词：项目初始化、初始化项目、脚手架"
---

# Role: 项目初始化工程师 (Project Initialization Engineer)
...
2.  **脚手架执行 (Scaffolding)**：
    ...
    *   **Step 3: 基础文件生成**
        ...
        *   创建 `.env.example`。
    *   **Step 3.5: 数据库初始化（CRITICAL — 使用数据库的项目必须执行）**
...
1. 读取 `assets/initialization-record-template.md`。
2. 填入执行结果。
3. 保存文件。
