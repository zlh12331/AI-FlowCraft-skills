...
description: "扫描项目组件源码，自动识别可测试的 UI 组件，编写并运行组件测试代码（渲染、交互、状态、快照），生成覆盖率报告。当用户提到"组件测试"、"component test"、"UI测试"、"快照测试"、"React Testing Library"、"Vue Test Utils"、"测试组件"时使用。"
---
...
**推荐**：React + RTL（生态成熟、官方维护）。其他选项参见 `examples/component-test-examples.md`。
...
| 脚本 | 用途 | 用法 |
|------|------|------|
| `scan_components.py` | 扫描项目组件，识别可测试组件 | `python resources/scan_components.py <项目路径> [输出JSON]` |
| `generate_component_tests.py` | 根据扫描结果生成组件测试代码 | `python resources/generate_component_tests.py <扫描结果JSON> <输出目录>` |
| `run_component_tests.py` | 运行组件测试并收集覆盖率 | `python resources/run_component_tests.py <项目路径> [--coverage]` |
...
```bash
# 扫描项目组件
python resources/scan_components.py <项目路径> ./component_scan_result.json
```
...
```bash
# 基于扫描结果生成测试代码骨架
python resources/generate_component_tests.py ./component_scan_result.json ./generated_component_tests/
```
...
```bash
# 运行组件测试并收集覆盖率
python resources/run_component_tests.py <项目路径> --coverage
```
...
```bash
python resources/generate_report.py ./component_test_result.json docs/测试报告/组件测试报告.md ./component_scan_result.json
```
...
### React + React Testing Library（推荐）
...
describe('LoginForm', () => {
  ...
  it('输入凭据后点击登录应调用 onSubmit', async () => {
...
> **更多框架示例**：Vue 3 + VTU、Angular + ATL、Svelte 的完整测试示例请参阅 [`examples/component-test-examples.md`](examples/component-test-examples.md)。
